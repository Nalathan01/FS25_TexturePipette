MapObjectFinderUIHelper = {}

function MapObjectFinderUIHelper.createSection(generalSettingsPage, i18nTitleId)
...
function MapObjectFinderUIHelper.updateFocusIds(element)
	if not element then
		return
	end
	element.focusId = FocusManager:serveAutoFocusId()
	for _, child in pairs(element.elements) do
		MapObjectFinderUIHelper.updateFocusIds(child)
	end
end
...
local function createElement(generalSettingsPage, template, id, i18nTextId, target, callbackFunc)
	...
	MapObjectFinderUIHelper.updateFocusIds(elementBox)
...
function MapObjectFinderUIHelper.createBoolElement(generalSettingsPage, id, i18nTextId, target, callbackFunc)
...
function MapObjectFinderUIHelper.createChoiceElement(generalSettingsPage, id, i18nTextId, i18nValueMap, target, callbackFunc, nillable)
...
function MapObjectFinderUIHelper.createRangeElement(generalSettingsPage, id, i18nTextId, minValue, maxValue, step, unit, target, callbackFunc, nillable)
...
function MapObjectFinderUIHelper.createControlsDynamically(settingsPage, sectionTitle, owningTable, controlProperties, prefix)
	owningTable.sectionTitle = MapObjectFinderUIHelper.createSection(settingsPage, sectionTitle)
	...
	for _, controlProps in ipairs(controlProperties) do
		...
		if controlProps.min ~= nil then
			uiControl = MapObjectFinderUIHelper.createRangeElement(
			settingsPage, id, id,
		...
		elseif controlProps.values ~= nil then
			uiControl = MapObjectFinderUIHelper.createChoiceElement(settingsPage, id, id, controlProps.values, owningTable, callback, controlProps.nillable)
			uiControl.values = controlProps.values			uiControl.nillable = controlProps.nillable
		else
			uiControl = MapObjectFinderUIHelper.createBoolElement(settingsPage, id, id, owningTable, callback)
		end
		...
		MapObjectFinderUIHelper.registerFocusControls(owningTable.controls)
		settingsPage.generalSettingsLayout:invalidateLayout()
	end
end


function MapObjectFinderUIHelper.registerFocusControls(controls)
...
function MapObjectFinderUIHelper.setupAutoBindControls(owningTable, targetTable, updateFunc)
	owningTable.populateAutoBindControls = function()
		for _, control in ipairs(owningTable.controls) do
			if control.autoBind then
				local value = MapObjectFinderUIHelper.getAutoBoundValueFromTable(control, targetTable)
				MapObjectFinderUIHelper.setControlValue(control, value)
			end
		end
	end
	...
	for _, control in ipairs(owningTable.controls) do
		if control.autoBind then
			...
			owningTable[callbackName] = function(self, newState)
				local newValue = MapObjectFinderUIHelper.getControlValue(control, newState)
				MapObjectFinderUIHelper.setAutoBoundValueInTable(control, newValue, targetTable)
				if updateFunc then
...
function MapObjectFinderUIHelper.getAutoBoundValueFromTable(control, targetTable)
...
function MapObjectFinderUIHelper.setAutoBoundValueInTable(control, value, targetTable)
	if control.subTable == nil then
...
function MapObjectFinderUIHelper.setRangeValue(control, value)
	local valueIndex
	if control.nillable and value == nil then
		valueIndex = 1
	else
		valueIndex = math.floor(((value - control.min) / control.step + 1) + 0.5)		if control.nillable then
...
function MapObjectFinderUIHelper.getRangeValue(control, controlState)
...
function MapObjectFinderUIHelper.setChoiceValue(control, value)
...
function MapObjectFinderUIHelper.getChoiceValue(control, controlState)
	if control.hasStrings then
		return controlState
	else
...
function MapObjectFinderUIHelper.setBoolValue(control, value)
	control.elements[1]:setState(value and BinaryOptionElement.STATE_RIGHT or BinaryOptionElement.STATE_LEFT)
end

function MapObjectFinderUIHelper.getBoolValue(controlState)
	return controlState == 2
end

function MapObjectFinderUIHelper.setControlValue(control, value)
	if control.min ~= nil then
		MapObjectFinderUIHelper.setRangeValue(control, value)
	elseif control.values ~= nil then
		MapObjectFinderUIHelper.setChoiceValue(control, value)
	else
		MapObjectFinderUIHelper.setBoolValue(control, value)
	end
end

function MapObjectFinderUIHelper.getControlValue(control, controlState)
	if control.min ~= nil then
		return MapObjectFinderUIHelper.getRangeValue(control, controlState)
	elseif control.values ~= nil then
		return MapObjectFinderUIHelper.getChoiceValue(control, controlState)
	else
		return MapObjectFinderUIHelper.getBoolValue(controlState)
	end
end
