...
local function tpGroundLayerLooksPaintable(layerName)
    ...
    return string.upper(layerName) == layerName
end
...
function MapObjectFinderGroundTextures:appendAvailableTerrainLayers(manager, terrain)
    if manager == nil
...
local function tpAppendGroundTextureMappings(manager, terrain)
    MapObjectFinderGroundTextures:appendAvailableTerrainLayers(manager, terrain)
end
