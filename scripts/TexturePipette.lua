...
TexturePipette.mouseX = 0.5
TexturePipette.mouseY = 0.5
TexturePipette.raycastHit = nil

TexturePipette.isPipetteArmed = false

TexturePipette.nextArmedStatusRefreshAt = 0
TexturePipette.armedStatusRefreshInterval = 1.75

TexturePipette.tpResultItems = {}
TexturePipette.tpConstructionCategoryRegistered = false
TexturePipette.tpLastTrackedManualSelectionKey = nil
TexturePipette.tpDecoratedPipetteItems = nil
TexturePipette.tpLastTrackedConstructionSelectionKey = nil
TexturePipette.tpLastConstructionSelectionSnapshot = nil

TexturePipette.lastPipetteWorldX = nil
TexturePipette.lastPipetteWorldY = nil
TexturePipette.lastPipetteWorldZ = nil
TexturePipette.tpDebugOverlayLines = {}
TexturePipette.tpDebugLastClickTime = 0

local TP_MOD_DIRECTORY = g_currentModDirectory or ""

local TP_DEBUG_LOG = false
local TP_EXTENDED_DEBUG = false
local TP_DEBUG_OVERLAY_ENABLED = true
local TP_FOLIAGE_INCLUDE_GROUND_RESULTS = true
...
local function tpLog(message)
    ...
    if isError == true or TP_DEBUG_LOG == true or (tpIsDebugModeEnabled ~= nil and tpIsDebugModeEnabled() == true) then
        print(string.format("[TexturePipette] %s", message))
    end
end

tpIsDebugModeEnabled = function()
    if TP_DEBUG_OVERLAY_ENABLED ~= true then
        return false
    end
...
function TexturePipette:tpRestorePipetteDecoratedNames()
...
function TexturePipette:tpDecoratePipetteResultNames(items)
...
function TexturePipette:tpFormatConstructionMenuDebugLabel(item, categoryIndex, tabIndex, itemIndex)
    if type(item) ~= "table" then
        return nil
    end
...
function TexturePipette:tpExposeConstructionInternalNames(screen)
...
function TexturePipette:loadMap()
    self.mouseX = 0.5
    self.mouseY = 0.5
    self.raycastHit = nil
...
function TexturePipette:tpGetConstructionIconUVs()
...
function TexturePipette:tpRegisterConstructionCategory()
    if TexturePipetteMenu ~= nil then
        if not TexturePipetteMenu:isEnabled("pipetteEnabled") then
            return false
        end
        return TexturePipetteMenu:registerConstructionMenu()
    end
    ...
    if tab == nil then
        g_storeManager:addConstructionTab(
            "TP_PIPETTE",
            "TP_RESULTS",
            tpText("TP_tab_results", "Detected Textures"),
            nil,
            nil,
            TP_MOD_DIRECTORY,
            nil
...
function TexturePipette:tpEnsurePipetteResultItemSlot(screen)
    if screen == nil
...
local function tpPipetteAppendRebuildData(self)
    if TexturePipette ~= nil then
        TexturePipette:tpRegisterConstructionCategory()
        TexturePipette:tpEnsurePipetteResultItemSlot(self)
    end
end
...
function TexturePipette:tpFindPipetteScreenIndices(screen)
...
function TexturePipette:tpIsPipetteResultTabActive(screen)
    local categoryIndex, tabIndex = self:tpFindPipetteScreenIndices(screen)
...
function TexturePipette:tpGetPipetteUiState(screen)
    if screen == nil then
        return nil
    end
...
function TexturePipette:tpUpdatePipettePanelVisuals(screen)
...
function TexturePipette:tpSetPipettePanelStatus(text)
    self.tpPipettePanelStatusText = tostring(text or "")
    self:tpUpdatePipettePanelVisuals(self:tpResolveConstructionLogicScreen())
end

function TexturePipette:tpClearPipettePanelStatus()
    self.tpPipettePanelStatusText = ""
    self:tpUpdatePipettePanelVisuals(self:tpResolveConstructionLogicScreen())
end

function TexturePipette:tpCreatePipettePanel(screen)
    local state = self:tpGetPipetteUiState(screen)
    if state == nil then
        return false
    end
    ...
    local candidateText = container:getDescendantById("tpPipetteStatusText")
...
function TexturePipette:tpRefreshPipetteResultItems(screen)
    local categoryIndex, tabIndex = self:tpFindPipetteScreenIndices(screen)
...
function TexturePipette:tpUpdatePipetteResultArea()
...
function TexturePipette:tpSetConstructionSelectorBrush(screen)
...
function ConstructionScreen:onPtpPipetteActivateButtonClick()
    ...
    local hasSelection = TexturePipette.lastPipetteWorldX ~= nil
    local wasArmed = TexturePipette.isPipetteArmed == true

    TexturePipette:tpSetConstructionSelectorBrush(self)

    if hasSelection then
        TexturePipette.isPipetteArmed = true
        TexturePipette.nextArmedStatusRefreshAt = 0
        TexturePipette:tpClearPipetteSelection(self, "buttonRearm")
        TexturePipette:tpUpdatePipettePanelVisuals(self)
        tpShowMessage(tpText("TP_msg_armed", "Pipette ready. Click target."))
        return
    end

    TexturePipette.isPipetteArmed = not wasArmed
    TexturePipette.nextArmedStatusRefreshAt = 0
    TexturePipette:tpUpdatePipettePanelVisuals(self)

    if TexturePipette.isPipetteArmed then
        TexturePipette:tpInstallShowDialogSuppressionHook()
        tpShowMessage(tpText("TP_msg_armed", "Pipette ready. Click target."))
    else
...
local function tpAfterConstructionScreenClose(screen, ...)
    ...
    TexturePipette.isPipetteArmed = false
    TexturePipette.nextArmedStatusRefreshAt = 0
    TexturePipette:tpClearPipetteSelection(screen, "constructionScreenClose")
end
...
function TexturePipette:mouseEvent(posX, posY, isDown, isUp, button)
...
function TexturePipette:tpGetCurrentPipetteSelectedItem(screen)
...
function TexturePipette:tpGetCurrentConstructionSelectedItem(screen)
...
function TexturePipette:tpBuildConstructionItemConfirmLabel(item, itemIndex, categoryIndex, tabIndex)
    if type(item) ~= "table" then
        return "-"
    end
...
function TexturePipette:tpTrackActiveConstructionSelection(context, force)
    local screen = self:tpResolveConstructionLogicScreen()
    ...
    if item.tpMapFoliageResult == true and tpIsDebugModeEnabled() == true then
        ...
        local wantedLayer = tostring(item.tpMapFoliageLayer or "")
        local wantedState = tostring(item.tpMapFoliageState or "")
        ...
        if force == true or self.tpLastMapFoliageSelectionLogKey ~= selectionLogKey then
            tpLog(string.format(
                ...
                tostring(item.tpMapFoliageLayer or "<nil>"),
                tostring(item.tpMapFoliageState or "<nil>"),
        ...
        self.tpLastMapFoliageItem = {
            context = tostring(context or "selection"),
            layer = tostring(item.tpMapFoliageLayer or ""),
            state = tostring(item.tpMapFoliageState or ""),
...
function TexturePipette:tpInstallConstructionInteractionTrace()
    if TP_EXTENDED_DEBUG ~= true then
        return
    end
...
function TexturePipette:tpTrackManualPipetteSelection()
end

function TexturePipette:update(dt)
    self:tpInstallShowDialogSuppressionHook()
    if TP_EXTENDED_DEBUG == true then
...
function TexturePipette:updatePersistentArmedStatus()
...
function TexturePipette:isConstructionScreenOpen()
    if g_gui == nil then
        return false
    end
...
function TexturePipette:tpResolveConstructionLogicScreen()
    local candidates = {}
...
function TexturePipette:tpCollectCurrentPaintTabCandidates()
    local screen = self:tpResolveConstructionLogicScreen()
        ...
        if type(categoryItems) == "table" then
            for tabIndex, tabItems in pairs(categoryItems) do
                if type(tabItems) == "table" then
                    for itemIndex, item in ipairs(tabItems) do
                        if type(item) == "table"
                            and (item.terrainOverlayLayer ~= nil or item.overlayLayer ~= nil or item.terrainLayer ~= nil)
                            and type(item.brushParameters) == "table"
                            and type(item.brushParameters[1]) == "string"
                            and item.brushParameters[1] ~= "" then

                            local terrainOverlayLayer = item.terrainOverlayLayer or item.overlayLayer or item.terrainLayer
...
function TexturePipette:raycastClosestCallback(nodeId, x, y, z, distance, nx, ny, nz, subShapeIndex, shapeId, isLast)
    self.raycastHit = {
        nodeId = nodeId,
        x = x,
        y = y,
...
function TexturePipette:tpResolveNodeObjectFromRaycastHit()
...
function TexturePipette:tpIsRaycastHitTerrainRelated()
    local hit = self.raycastHit
    local nodeId = hit ~= nil and hit.nodeId or nil
...
function TexturePipette:tpGetRaycastHitNodeLabel()
    local hit = self.raycastHit
    local nodeId = hit ~= nil and hit.nodeId or nil
    if nodeId == nil or nodeId == 0 then
        return "object"
...
function TexturePipette:tpTryHandleStaticMapObjectHit(screen)
...
function TexturePipette:tpGetDisplayItemStoreFilename(item)
    if type(item) ~= "table" then
        return nil
    end
...
function TexturePipette:tpFindConstructionDisplayItemForStoreItem(screen, storeItem, xmlFilename)
    if screen == nil or type(screen.items) ~= "table" then
...
function TexturePipette:tpResetLayerMenuOutput()
end


function TexturePipette:tpResolveStoreItemFromPlaceableObject(object)
...
function TexturePipette:tpTryPickPlaceableAtCurrentRaycast()
    ...
    if displayItem == nil then
        local storeName = tpCleanPanelObjectLabel(storeItem.name or storeItem.customEnvironment or storeItem.xmlFilename or "Object")
        self.tpResultItems = {}
        self:tpResetLayerMenuOutput()
        self.tpPipettePanelStatusText = string.format(tpText("TP_msg_objectNotReady", "Not buildable: %s"), storeName)
        self:tpRefreshPipetteResultItems(screen)
        self:tpUpdatePipettePanelVisuals(screen)
        tpLog("placeableStoreItemNotSelectable name=" .. tostring(storeName) .. " reason=notInConstructionScreen")
        return true
    end
...
function TexturePipette:findMouseWorldPosition()
...
function TexturePipette:getTerrainRoot()
...
function TexturePipette:tpRankCandidateMatchesBySubLayerCompetition(candidateMatches, sessionId)
...
function TexturePipette:tpTryPreselectFirstPipetteResult(screen)
...
function TexturePipette:tpStoreResultMatchesForResultTab(mergedMatches)
    self:tpRestorePipetteDecoratedNames()
    local resultItems = {}
...
function TexturePipette:tpClearPipetteSelection(screen, reason)
    self:tpRestorePipetteDecoratedNames()
    self.tpResultItems = {}
    self.tpPipettePanelStatusText = ""
    self:tpResetLayerMenuOutput()
...
function TexturePipette:tpFormatDebugValue(value)
    local valueType = type(value)
...
function TexturePipette:tpLogTableKeys(label, data, maxKeys)
...
function TexturePipette:tpLogCandidateDeep(label, item, maxKeys)
...
function TexturePipette:tpLogFoliageFunctionAvailabilityOnce()
...
function TexturePipette:tpLogConstructionStructureOnce(screen)
...
function TexturePipette:tpFunctionSourceLabel(fn)
...
function TexturePipette:tpCollectKeys(object, maxKeys)
    local keys = {}
    if type(object) == "table" then
...
function TexturePipette:tpLogRuntimeObject(label, object, maxKeys)
...
function TexturePipette:tpLogConstructionBrushRuntime(screen, context)
...
function TexturePipette:tpLogFoliageMenuItemInternals(screen, context)
...
function TexturePipette:tpExtractNibbleStatesFromDensity(rawValue)
    local states = {}
    local seen = {}
...
function TexturePipette:tpCollectFoliageMenuCandidatesAtCurrentPick(screen)
    local x = self.lastPipetteWorldX
    local y = self.lastPipetteWorldY
    local z = self.lastPipetteWorldZ
    ...
    if TP_EXTENDED_DEBUG == true then
        tpLog("visualFoliageMappingDebugActive=true")
    ...
    local function tpMapFoliageCloneTable(source, depth)
        ...
        for key, value in pairs(source) do
            if key ~= "tpPipetteOriginalDisplayName" and key ~= "tpPipetteDebugSuffix" and key ~= "tpPipetteConfirmLabel" then
                if type(value) == "table" then
                    clone[key] = tpMapFoliageCloneTable(value, depth + 1)
                else
                    clone[key] = value
                end
            end
    ...
    local function tpBuildMapFoliageItem(unresolvedLayer, state)
        ...
        local item = tpMapFoliageCloneTable(template, 0)
        item.name = "Karten-Foliage: " .. layerName .. " | " .. tostring(numericState)
        item.title = item.name
        item.price = 0
        item.dailyUpkeep = 0
        item.brushParameters = { layerName, tostring(numericState) }
        item.tpMapFoliageResult = true
        item.tpMapFoliageLayer = layerName
        item.tpMapFoliageState = tostring(numericState)
        item.tpMapFoliageSourceStates = tostring(unresolvedLayer.states or "")
        item.tpPipetteDebugSuffix = nil
    ...
    local function buildUnresolvedVisualBushReviewMatches()
        ...
        local function logUnsupportedVisualLayer(unresolvedLayer)
            ...
            local decision = "map_foliage_not_directly_paintable"
            ...
            tpLog("mapFoliageFound layer=" .. visualLayerName .. " states=" .. tostring(unresolvedLayer.states or "<nil>") .. " score=" .. tostring(unresolvedLayer.weightedScore or 0) .. " menuLayerMatch=false paintableByConstruction=false nextStep=inspect_map_foliage_definition")
            self.tpMapFoliageFoliageMarker = {
            ...
            tpLog("mapFoliageMarkerCreated layer=" .. visualLayerName .. " states=" .. tostring(unresolvedLayer.states or "<nil>") .. " duration=20")
            ...
            tpShowMessage(string.format(tpText("TP_msg_notInBuildMenu", "No selectable construction item found for: %s"), tostring(visualLayerName or "?")))
        end

        local function addMapFoliageItems(unresolvedLayer)
            if unresolvedLayer == nil then
                return
            end
            local states = {}
            ...
            for _, state in ipairs(states) do
                local item = tpBuildMapFoliageItem(unresolvedLayer, state)
                if item ~= nil then
    ...
    if unresolvedVisualBushCount > 0 and exactMatchCount == 0 and #layerReviewMatches == 0 then
        ...
        tpLog("foliageSelectionMode=unmappedVisualBushOnlyMapFoliageProbe unresolvedVisualBushLayers=" .. tostring(unresolvedVisualBushCount) .. " exact=" .. tostring(exactMatchCount) .. " selected=" .. tostring(#selected) .. " mapFoliageItems=" .. tostring(#unresolvedVisualBushReviewMatches) .. " review=" .. tostring(#layerReviewMatches))
    elseif unresolvedVisualBushCount > 0 then
        ...
        tpLog("foliageSelectionMode=unmappedVisualBushSupportedOnly unresolvedVisualBushLayers=" .. tostring(unresolvedVisualBushCount) .. " exact=" .. tostring(exactMatchCount) .. " selected=" .. tostring(#selected) .. " mapFoliageItems=" .. tostring(#unresolvedVisualBushReviewMatches) .. " reviewSuppressed=" .. tostring(#layerReviewMatches))
    ...
    local function tpAddStrongVisibleLayerCandidates()
        ...
        local added = 0
        local maxAdditional = 5
        local minScore = math.max(6, bestLayerScore * 0.22)
    ...
    if #selected > 0 then
        ...
        for _, match in ipairs(selected) do
            ...
            if perLayer[layerName] < 2 and #limited < 6 then
                table.insert(limited, match)
                perLayer[layerName] = perLayer[layerName] + 1
            end
        end
        if #limited ~= #selected then
            tpLog("balancedFoliageDisplayLimit before=" .. tostring(#selected) .. " after=" .. tostring(#limited) .. " maxTotal=6 maxPerLayer=2")
        end
        selected = limited
    end
    ...
    for _, match in ipairs(selected) do
        if match ~= nil and match.sourceItem ~= nil then
            if match.mapFoliageResult == true then
            ...
            elseif string.find(string.lower(tostring(match.layerName or "")), "bush", 1, true) ~= nil then
                ...
                if isPrimary then
                    match.sourceItem.tpPipetteDebugSuffix = " [State " .. tostring(match.state or "?") .. " | direct" .. imageName .. "]"
                else
            ...
            elseif match.reviewBroad == true then
                ...
                match.sourceItem.tpPipetteDebugSuffix = " [" .. tostring(match.layerName or "?") .. " | State " .. tostring(match.state or "?") .. imageName .. "]"
            else
                match.sourceItem.tpPipetteDebugSuffix = nil
            end
        end
...
function TexturePipette:tpFormatTraceValue(value)
    if value == nil then
        return "<nil>"
    end
...
function TexturePipette:tpSafeCallTrace(functionName, ...)
...
function TexturePipette:tpBuildFoliageTraceMenuMap(screen)
    local map = {}
    local treeItems = {}
...
function TexturePipette:tpCollectTraceSamples(planeId, rawX, rawY, rawZ, gridX, gridZ)
    local samples = {}
    local offsets = {
...
function TexturePipette:tpLogOfficialFoliageTraceAtCurrentPick(screen)
...
function TexturePipette:tpProbeFoliageAtCurrentPick()
    local x = self.lastPipetteWorldX
    local y = self.lastPipetteWorldY
    local z = self.lastPipetteWorldZ
    local foliageSystem = g_currentMission ~= nil and g_currentMission.foliageSystem or nil
...
function TexturePipette:tpLogFoliageStructureOnce()
...
function TexturePipette:tpBuildFoliageDebugOverlayForWorld(x, y, z)
    if tpIsDebugModeEnabled() ~= true then
        return
    end

    local lines = {}
    table.insert(lines, "Texture Pipette Debug")
...
function TexturePipette:tpDrawDebugWorldLine(x1, y1, z1, x2, y2, z2, r, g, b)
    r = r or 0.15
    g = g or 1.0
    b = b or 0.15
...
function TexturePipette:tpResolveDebugFoliagePlane()
    local foliageSystem = g_currentMission ~= nil and g_currentMission.foliageSystem or nil
    if type(foliageSystem) ~= "table" then
        return nil, nil
    end
...
function TexturePipette:tpSampleDebugDensity(planeId, x, y, z)
    if type(getDensityAtWorldPos) ~= "function" or planeId == nil then
        return nil
    end
...
function TexturePipette:tpFindDebugRasterBoundary(planeId, x, y, z, dx, dz, maxDistance, stepSize)
...
function TexturePipette:tpBuildAlignedDebugGridPosition(x, y, z)
...
function TexturePipette:tpDrawPersistentDebugGrid()
    ...
    if anyLine == true then
        local modeText = grid.boundaryMode == "officialCell" and "official LS25 cell size" or (grid.aligned and "measured LS25 boundary" or "fallback boundary")
        if recognitionFallbackActive == true then
            modeText = "Fallback: keine sichere Foliage-Erkennung"
        end
        self.tpDebugOverlayLines = {
            "Texture Pipette Debug",
            "3x3 Grid: " .. modeText,
...
function TexturePipette:tpDrawDebugOverlayNow(source)
    if tpIsDebugModeEnabled() ~= true then
        return
    end
    ...
    if type(lines) ~= "table" or #lines == 0 then
        lines = { "Texture Pipette Debug", "3x3 grid active.", "Use the pipette to inspect values." }
    end
    ...
    pcall(function()
        renderText(0.02, 0.76, 0.014, gridDrawn and "Debug grid: visible (" .. tostring(source or "world") .. ")" or "Debug grid: 3D line drawing unavailable")
    end)
end


function TexturePipette:tpDrawMapFoliageFoliageMarkerNow()
    if type(self.tpMapFoliageFoliageMarker) ~= "table" then
        return
    end

    local marker = self.tpMapFoliageFoliageMarker
    local now = getTimeSec ~= nil and getTimeSec() or 0
    if tonumber(marker.expiresAt or 0) < now then
        self.tpMapFoliageFoliageMarker = nil
        return
    end
...
function TexturePipette:draw()
    self:tpDrawDebugOverlayNow("mod")
    self:tpDrawMapFoliageFoliageMarkerNow()
end

local function tpAfterConstructionScreenDraw(screen, ...)
    if TP_DEBUG_OVERLAY_ENABLED == true and TexturePipette ~= nil and TexturePipette.tpDrawDebugOverlayNow ~= nil then
        TexturePipette:tpDrawDebugOverlayNow("construction")
    end
end
...
function TexturePipette:pickTextureAtCurrentMousePosition()
    local x, y, z = self:findMouseWorldPosition()
   ...
   if type(self.tpLastMapFoliagePaintAttempt) == "table" then
    ...
    if self:tpTryPickPlaceableAtCurrentRaycast() then
        self.nextArmedStatusRefreshAt = 0
        return
    end
    ...
    local groundMenuItems = {}

    if TP_FOLIAGE_INCLUDE_GROUND_RESULTS == true then
        for _, entry in ipairs(rankedMatches or {}) do
            local item = entry ~= nil and entry.sourceItem or nil
            if item ~= nil then
                table.insert(groundMenuItems, item)
            end
        end
    else
    ...
    local function appendResultItems(items, limit)
        local added = 0
        for _, item in ipairs(items or {}) do
            if item ~= nil and usedFinalItems[item] ~= true then
                ...
                added = added + 1
                if limit ~= nil and added >= limit then
                    break
                end
                if #finalResultItems >= 8 then
                    break
                end
            end
        end
    ...
    if #layerMenuItems > 0 and #groundMenuItems > 0 then
        appendResultItems(layerMenuItems, 4)
        appendResultItems(groundMenuItems, 4)
        if #finalResultItems < 8 then
            appendResultItems(layerMenuItems, 8 - #finalResultItems)
        end
        if #finalResultItems < 8 then
            appendResultItems(groundMenuItems, 8 - #finalResultItems)
        end
    else
        appendResultItems(layerMenuItems, 8)
        appendResultItems(groundMenuItems, 8)
    end
...
function TexturePipette:tpArmPipetteWorldClickDialogSuppression()
    self.tpSuppressNextObjectInfoDialog = true
    local now = getTimeSec ~= nil and getTimeSec() or 0
    self.tpSuppressNextObjectInfoDialogUntil = now + 0.75
end

function TexturePipette:tpInstallShowDialogSuppressionHook()
    ...
    source.showDialog = function(gui, ...)
        if TexturePipette ~= nil and TexturePipette.tpSuppressNextObjectInfoDialog == true then
            local now = getTimeSec ~= nil and getTimeSec() or 0
            local untilTime = tonumber(TexturePipette.tpSuppressNextObjectInfoDialogUntil) or 0

            if now <= untilTime then
                TexturePipette.tpSuppressNextObjectInfoDialog = false
                TexturePipette.tpSuppressNextObjectInfoDialogUntil = 0
                return
            end
...
function TexturePipette:tpClearExpiredObjectInfoDialogSuppression()
...
addModEventListener(TexturePipette)
